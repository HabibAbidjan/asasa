...
def handle_mines_click(call):
    ...
    if len(state['opened']) >= 22:
        state['active'] = False
        win = state['bet'] * 2
        user_balances[user_id] += win
        send_mines_grid(user_id, state['bet'], state['mines'], state['opened'], active=False, win_amount=win, chat_id=call.message.chat.id, msg_id=call.message.message_id)
        return
...
def send_mines_grid(user_id, bet, mines, opened, active=True, bomb_hit=False, win_amount=0, chat_id=None, msg_id=None):
    ...
    if active:
        for i in range(25):
            if i not in opened:
                markup.add(types.InlineKeyboardButton("⬜", callback_data=f"mines_{i}"))
        markup.row(types.InlineKeyboardButton("💸 Cash Out", callback_data="mines_cashout"))
